...
class Generator(nn.Module):
    def __init__(self, latent_dim, airfoil_dim):
        ...
        # output dim: (batch_size, airfoil_dim)
        # uses tanh() as the activation for the last layer
        # since y coord of airfoils range from -1 to 1

        self.latent_dim = latent_dim
        self.airfoil_dim = airfoil_dim

        # defines one "block" of linear/batchnorm/leakyReLU layers
    ...
    # Input shape: (batch_size x latent_dim)
    # Output shape: (batch_size x airfoil_dim)
    def forward(self, x):
