...
def load_files_train(label, group, X, y):
    path = group + '/' +label + '/'
...
def load_files_test(label, group, X):
    path = group + '/' +label + '/'
    files = os.listdir(path)
    # Load (x, y))
    for file in files:
...
def load_dataset():
    ...
    for label in output_labels:
        trainX, trainy = load_files_train(label, group, trainX, trainy)
    trainX = np.expand_dims(trainX, axis=2)
   
    #Load Test data (X only, no y)
    group = "test"
...
def evaluate_model(trainX, trainy, testX):
...
def make_predictions():
    ...
    # repeat experiment
    model, batch_size = evaluate_model(trainX, trainy, testX)
